...
| 10 | **Burst detection measures archive backfill, not publication cadence.** Rotten Tomatoes imported long-serving critics' back catalogues under a single timestamp, and a RANGE window includes every row tied on the ordering value. The original notebook's row-wise loop has the same behaviour. The consequence is perverse: the most prolific and most respected critics score as the most suspicious. The first full run rated Roger Ebert at **2,159 reviews in 7 days**. | cell 54; surfaced by the first real run of this rework | `int_burst_activity` counts same-day reviews per critic first, flags any date carrying more than `backfill_same_day_threshold` (default 8) as `is_backfill_batch`, and excludes those rows from the window. They get NULL burst features rather than an invented number. |
| 11 | **Placeholder publication dates taken at face value.** The dump's earliest `review_date` is 1800-01-01. Left as-is these feed `days_since_release` and the burst windows. | cell 33; surfaced by the first real run | Dates outside `[earliest_plausible_review_date, today]` are nulled in staging. The review is kept — its text is still usable by Models 1 and 2 — while the timing features correctly report "unknown". |
...
| # | Finding | Where | Resolution |
|---|---|---|---|
| 1 | **Potential target leakage into Model 2.** The cleaner strips punctuation but keeps digits, so a review reading *"Verdict: 3/5"* contributes the token `35` — the target, sitting in the input. | cells 29, 50 | `clean_text` removes digits. **Measured, the effect turned out to be immaterial on this dataset:** only 0.2% of top-critic reviews restate a numeric score in the body, and on those rows the difference is within noise (41 holdout rows). The mechanism is real and the fix is still the right default, but this is not a leak that was inflating the original results, and section 4 of the notebook reports it as the negative result it is. |
...
| **Reviews with no date, or a placeholder one.** | Burst windows and `days_since_release` are undefined. The dump's earliest date is 1800-01-01, which is a sentinel rather than a publication date. | Implausible dates are nulled in staging; undated rows are excluded from `int_burst_activity` by its own filter. The mart left-joins, so the features arrive as NULL and are zero-filled at the model boundary rather than silently defaulting mid-pipeline. |
| **Archive backfill batches** — a critic's back catalogue imported under one timestamp. | The single most important behavioural feature stops measuring behaviour: see Methodology #10. | Detected per (critic, date) and excluded from the burst window, with the flag carried into the mart so the analysis can see how much data it covers. |
